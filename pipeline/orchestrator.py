...
Coordinates the entire flow:
    Step 1 (on upload):  Index video → transcript + visual tags + scenes → metadata
...
class VideoRAGOrchestrator:
    ...
    def index_video(self, video_path: str, video_id: str, extra: Dict = None) -> Dict:
        """
        Process a newly uploaded video through Step 1: Low-Cost Indexing.

        Extracts:
        - Audio → Whisper transcript (timestamped segments)
        - Visual tags → YOLO object detection (timestamped labels)
        - Scene boundaries → PySceneDetect

        Then builds unified metadata and indexes it into Qdrant.
        ...
        # 1f. Chunk and embed metadata into Qdrant
